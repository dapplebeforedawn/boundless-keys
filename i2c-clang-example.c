        ...
        switch (argv[argnum][1]) {
            ...
            case 'd':  // Read/Write Mode
                switch (argv[argnum][2]) {
                    ...
                    case 'p': mode = MODE_READ_PARTIAL; break;
            ...
            case 's':  // Slave address
                ...
                break;
            case 'r':  // Slave register
                regaddr = atoi(argv[argnum]+2);
                break;
            default:
...
int main(int argc, char **argv) {
    ...
    if (mode == MODE_READ_PARTIAL) {
      printf("Read Partial: %x\n", regaddr);
      for (i=0; i<len; i++) buf[i] = 'n';
      data = bcm2835_i2c_read_register_rs(&regaddr, buf, len);
      printf("Read Result = %d\n\n", data);
      for (i=0; i<len; i++) {
              if(buf[i] != 'n') printf("Read Buf[%d] = %x\n", i, buf[i]);
      }
